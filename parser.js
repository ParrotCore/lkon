const 
    ...
    errors = {
        ...
        wrongSyntax: (line, position) => new SyntaxError(`Something is wrong with syntax at ${Number(line)}:${position}`),
        unexpectedValue: (line, position, value) => new TypeError(`Unexpected '${value}' value at ${Number(line)+1}:${position}`),
        unexpectedEnd: (line, position) => new SyntaxError(`Unexpected end of LKON at ${line}:${position}`),
        keywordError: (line, position, name) => new SyntaxError(`Name '${name}' is reserved for keywords at ${line}:${position}`),
        keyError: (line, position, name) => new TypeError(`Key name '${name}' is reserved for LKON ${name.replace(/^__/, '')} object at ${line}:${position}.`),
        reservedProperty: (line, position, name) => new TypeError(`Key name '${name}' is reserved for LKON data-${name.replace(/^__/, '')} property at ${line}:${position}`),
        variablesConflict: (line, position, name, type) => new Error(`'${name}' key has already been reserved for ${["variable","import"][type]} at ${line}:${position}`)
    },
...
function getImport(line, lineNumber, imports, variables)
{
    ...
    if(encoding == 'bin') throw TypeError(`Cannot parse binary file at ${lineNumber}:${line.indexOf('bin')}`);
    let value = lkonParse(parseFile(`"${path}"${encoding}`));
...
function lkonParse(lkonData)
{
    ...
    {
        if(!startMain && !['[', '[];'].includes(lines[i].replace(/[\t ]+/g, '')) && lines[i].length)
        {
            ...
            else if(lines[i].replace(/[\t ]/g, ''))
            {
                ...
                else throw errors.wrongSyntax(i, lines[i].indexOf(lines[i].replace(/[\t ]+/g, '')[0]))
            }
        }
        else if(!startMain && lines[i].replace(/[\t ]+/g, '') == '[];')
        {
            startMain = true;
            endMain = true;
        }
        ...
        {
            if(endMain && lines[i].replace(/[\t ]+/g, '') != '') throw errors.unexpectedEnd(i, lines[i].indexOf(']'));
            else if(lines[i].replace(/[\t ]/g, '') == '];')
            {
            ...
            else if(lines[i].replace(/[\t ]/g, '') != '')
            {
                ...
                {
                    if(!lines[i].endsWith(';') && !lines[i].endsWith('[')) return errors.missingSemicolon(i, lines[i].length)
